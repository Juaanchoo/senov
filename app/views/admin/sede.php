	    ...
	    <div class="card-body">
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody class="text-center">
                        ...
                        <tr>
                            <td><?php echo $item->id_sede ?></td>
                            <td><?php echo $item->sede ?></td>
                            <td>

                                <button type="button" class="btn btn-warning" onclick="traerSede(<?php echo $item->id_sede ?>)">
                                    <i class="far fa-edit"></i>
                                </button>

                            <?php if ($item->estado == 1): ?>
                            
                                <button type="button" class="btn btn-danger" onclick="eliminarTipoFormacion(<?php echo $item->id_sede ?>,0)">
                                    <i class="far fa-trash-alt"></i>
                                </button>

                            <?php else: ?>
                                <button type="button" class="btn btn-success" onclick="eliminarTipoFormacion(<?php echo $item->id_sede ?>,1)">
                        ...
                        <div class="row border rounded bg-white shadow" style="height: 50%; width:95%; margin-left:2.5%;">
                            ...
                            <div class="col-12 app-p">
                                <div class="form-group">
                                    ...
                                    <input id="nombreSede" type="text" class="form-control" name="nombreSede">
...
function traerSede(id){
    $.ajax({
        url: "http://localhost/senov/admin/ver_sede?id="+id,
        type: 'GET',
        success: function(res){
            res = JSON.parse(res).data;
            $('#nombreSede').val(res.sede);
...
function eliminarTipoFormacion(id, estado) {
    $.ajax({
        url: "http://localhost/senov/admin/sede?estado="+estado+"&id="+id,
        type: 'DELETE',
        success: function(res){
            res = JSON.parse(res).data;
...
$("#btn-registrar").click(function(){
  var nombreSede = $('#nombreSede').val();
    ...
    $.post( "http://localhost/senov/admin/sede", {nombre: nombreSede})
        .done(function( data ) {
...
$("#btn-actualizar").click(function(){
  ...
  var id = $('#id_sede').val();
    ...
    $.ajax({
        url: "http://localhost/senov/admin/sede?nombre="+nombreSede+"&id="+id,
        type: 'PUT',
        success: function(res){
            res = JSON.parse(res).data;
            console.log(res);
