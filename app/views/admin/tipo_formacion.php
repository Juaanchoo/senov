...
<div class="container mt-5 app-m">
	<div class="card shadow">
	    ...
	    <div class="card-body">
            <div class="row">
                <div class="col-md-6">
                    <a onclick="limpiarModal()" href="" data-toggle="modal" data-target="#exampleModal" class="btn btn-outline-dark">Registrar Tipo de Fomación &nbsp;<i class="fas fa-plus"></i></a>
                </div>
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody class="text-center">
                        ...
                        <tr>
                            <td><?php echo $item->id_tipo_formacion ?></td>
                            <td><?php echo $item->tipo_formacion ?></td>
                            <td>

                            <button type="button" class="btn btn-warning" onclick="traerjornada(<?php echo $item->id_tipo_formacion ?>)">
                                <i class="far fa-edit"></i>
                            </button>

                            <?php if ($item->estado == 1): ?>
                            
                                <button type="button" class="btn btn-danger" onclick="eliminarTipoFormacion(<?php echo $item->id_tipo_formacion ?>,0)">
                                    <i class="far fa-trash-alt"></i>
                                </button> 

                            <?php else: ?>
                                <button type="button" class="btn btn-success" onclick="eliminarTipoFormacion(<?php echo $item->id_tipo_formacion ?>,1)">
                        ...
                        <div class="row border rounded bg-white shadow" style="height: 50%; width:95%; margin-left:2.5%;">
                            ...
                            <div class="col-12 app-p">
                                <div class="form-group">
                                    <label for="nombreTF">Nombre Tipo de Formacion</label>
                                    <input id="nombreTF" type="text" class="form-control" name="nombreTF">
                                    <input id="id_tipo_formacion" type="hidden" class="form-control" name="nombreTF">
                                </div>
                            </div>
                        </div>
                    </div>
...
function limpiarModal() {
    $('#nombreTF').val('');
    $('#id_tipo_formacion').val('');
    $('#btn-registrar').show();         
    $('#btn-actualizar').hide();         

}
function traerjornada(id){
    $.ajax({
        url: "http://localhost/senov/admin/ver_tipo_formacion?id="+id,
        type: 'GET',
        success: function(res){
            res = JSON.parse(res).data;
            console.log(res);
            
            $('#nombreTF').val(res.tipo_formacion);
            $('#id_tipo_formacion').val(res.id_tipo_formacion);
...
function eliminarTipoFormacion(id, estado) {
    $.ajax({
        url: "http://localhost/senov/admin/tipo_formacion?estado="+estado+"&id="+id,
        type: 'DELETE',
        success: function(res){
            res = JSON.parse(res).data;
...
$("#btn-registrar").click(function(){
  var nombreTF = $('#nombreTF').val();
    ...
    $.post( "http://localhost/senov/admin/tipo_formacion", {nombre: nombreTF})
        .done(function( data ) {
...
$("#btn-actualizar").click(function(){
    ...
    $.ajax({
        url: "http://localhost/senov/admin/tipo_formacion?nombre="+nombreTF+"&id="+id,
        type: 'PUT',
        success: function(res){
            res = JSON.parse(res).data;
            console.log(res);
