...
<div class="container mt-5 app-m">
	<div class="card shadow">
	    <h5 class="card-header t-card">Gestion de Modalidad</h5>
	    <div class="card-body">
            <div class="row">
                <div class="col-md-6">
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody class="text-center">
                        ...
                        <tr>
                            <td><?php echo $item->id_modalidad ?></td>
                            <td><?php echo $item->modalidad ?></td>
                            <td>

                                <button type="button" class="btn btn-warning" onclick="traermodalidad(<?php echo $item->id_modalidad ?>)">
                                    <i class="far fa-edit"></i>
                                </button>

                            <?php if ($item->estado == 1): ?>
                            
                                <button type="button" class="btn btn-danger" onclick="eliminar(<?php echo $item->id_modalidad ?>,0)">
                                    <i class="far fa-trash-alt"></i>
                                </button>

                            <?php else: ?>
                                <button type="button" class="btn btn-success" onclick="eliminar(<?php echo $item->id_modalidad ?>,1)">
                        ...
                        <div class="row border rounded bg-white shadow" style="height: 50%; width:95%; margin-left:2.5%;">
                            ...
                            <div class="col-12 app-p">
                                <div class="form-group">
                                    ...
                                    <input id="nombremodalidad" type="text" class="form-control" name="nombremodalidad">
                                    <input id="id_modalidad" type="hidden">
                                </div>
                            </div>
                        </div>
                    </div>
...
function traermodalidad(id){
    $.ajax({
        url: "http://localhost/senov/admin/ver_modalidad?id="+id,
        type: 'GET',
        success: function(res){
            res = JSON.parse(res).data;
            $('#nombremodalidad').val(res.modalidad);
            $('#id_modalidad').val(res.id_modalidad);
...
function eliminar(id, estado) {
    $.ajax({
        url: "http://localhost/senov/admin/modalidad?estado="+estado+"&id="+id,
        type: 'DELETE',
        success: function(res){
            res = JSON.parse(res).data;
...
$("#btn-registrar").click(function(){
  var nombremodalidad = $('#nombremodalidad').val();
    ...
    $.post( "http://localhost/senov/admin/modalidad", {nombre: nombremodalidad})
        .done(function( data ) {
...
$("#btn-actualizar").click(function(){
  ...
  var id = $('#id_modalidad').val();
    ...
    $.ajax({
        url: "http://localhost/senov/admin/modalidad?nombre="+nombremodalidad+"&id="+id,
        type: 'PUT',
        success: function(res){
            res = JSON.parse(res).data;
            console.log(res);
