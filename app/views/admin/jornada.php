...
<div class="container mt-5 app-m">
	<div class="card shadow">
	    <h5 class="card-header t-card">Gestion de Jornada</h5>
	    <div class="card-body">
            <div class="row">
                <div class="col-md-6">
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody class="text-center">
                        ...
                        <tr>
                            <td><?php echo $item->id_jornada ?></td>
                            <td><?php echo $item->jornada ?></td>
                            <td>

                                <button type="button" class="btn btn-warning" onclick="traerjornada(<?php echo $item->id_jornada ?>)">
                                    <i class="far fa-edit"></i>
                                </button>

                            <?php if ($item->estado == 1): ?>
                            
                                <button type="button" class="btn btn-danger" onclick="eliminar(<?php echo $item->id_jornada ?>,0)">
                                    <i class="far fa-trash-alt"></i>
                                </button>

                            <?php else: ?>
                                <button type="button" class="btn btn-success" onclick="eliminar(<?php echo $item->id_jornada ?>,1)">
                        ...
                        <div class="row border rounded bg-white shadow" style="height: 50%; width:95%; margin-left:2.5%;">
                            ...
                            <div class="col-12 app-p">
                                <div class="form-group">
                                    <label for="nombrejornada">Nombre jornada</label>
                                    <input id="nombrejornada" type="text" class="form-control" name="nombrejornada">
                                    <input id="id_jornada" type="hidden">
                                </div>
                            </div>
                        </div>
                    </div>
...
function traerjornada(id){
    $.ajax({
        url: "http://localhost/senov/admin/ver_jornada?id="+id,
        type: 'GET',
        success: function(res){
            res = JSON.parse(res).data;
            $('#nombrejornada').val(res.jornada);
            $('#id_jornada').val(res.id_jornada);
...
function eliminar(id, estado) {
    $.ajax({
        url: "http://localhost/senov/admin/jornada?estado="+estado+"&id="+id,
        type: 'DELETE',
        success: function(res){
            res = JSON.parse(res).data;
...
$("#btn-registrar").click(function(){
  var nombrejornada = $('#nombrejornada').val();
    ...
    $.post( "http://localhost/senov/admin/jornada", {nombre: nombrejornada})
        .done(function( data ) {
...
$("#btn-actualizar").click(function(){
  ...
  var id = $('#id_jornada').val();
    ...
    $.ajax({
        url: "http://localhost/senov/admin/jornada?nombre="+nombrejornada+"&id="+id,
        type: 'PUT',
        success: function(res){
            res = JSON.parse(res).data;
            console.log(res);
